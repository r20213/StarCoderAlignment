...
class DPODataGenConfig:
    ...
    train_start_idx: Optional[int] = 20_000
    train_count: Optional[int] = None
    val_start_idx: Optional[int] = 0
...
# GENERATION ENGINE
# =========================================================
def generate_rejected_responses(model, tokenizer, dataset, cfg: DPODataGenConfig, start_idx: int = 0, max_count: Optional[int] = None) -> Dataset:
    generated_records = []
    ...
    for i in tqdm(range(start_idx, start_idx + total_samples), desc="Generating rejected responses"):
...
def main():
    ...
    dpo_train = generate_rejected_responses(model, tokenizer, raw_train, cfg, start_idx=cfg.train_start_idx, max_count=cfg.train_count)
    
    print("\n--- Processing Validation Split ---")
    dpo_val = generate_rejected_responses(model, tokenizer, raw_val, cfg, start_idx=cfg.val_start_idx, max_count=cfg.val_count)
